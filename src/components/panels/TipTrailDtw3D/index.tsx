// components/panels/TipTrailDtw3D
//
// v4.5: Two tip trails unfolded along normalized time axis + DTW connection lines.
...
function Scene({ data }: { data: DtwComparisonData }) {
  ...
  const { pts1, pts2 } = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) {
      p1.push(new THREE.Vector3(rec1.tipX[i], rec1.tipY[i] + Y_OFFSET, rec1.tn[i] * 3 - 1.5));
      p2.push(new THREE.Vector3(rec2.tipX[i], rec2.tipY[i] - Y_OFFSET, rec2.tn[i] * 3 - 1.5));
    }
  ...
  return (
    <>
      ...
      <FloorGrid />
      <DtwLinks path={dtwTip.path} pts1={pts1} pts2={pts2} />
...
export default function TipTrailDtw3D({ data }: { data: DtwComparisonData }) {
  return (
